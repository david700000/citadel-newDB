  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <body>
        ...
        <script>setTimeout(function() { window.print(); setTimeout(function() { window.close(); }, 500); }, 500);</script>
      </body>
      </html>
    `;
...
const CMSEventRegistrations = ({ state, toast }) => {
  ...
  const fetchRegistrations = async (silent = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!silent) toast("Failed to fetch event registrations", "error");
      }
    } catch (err) {
      if (!silent) toast("Connection error fetching registrations", "error");
    } finally {
  ...
  useEffect(() => {
    fetchRegistrations();
    fetchWebData();
    // Poll every 10 seconds so scanner check-ins appear automatically
    const interval = setInterval(() => fetchRegistrations(true), 10000);
  ...
  if (selectedEvent) {
    ...
    if (selectedDayTab === "all" || eventDaysArr.length === 0) {
      ...
      tabAttendedCount = eventRegs.filter(r => r.attendanceRecords && r.attendanceRecords.some(rec => rec.startsWith(selectedDayTab))).length;
    ...
    const attendedList = filtered.filter(r => {
      ...
      return r.attendanceRecords && r.attendanceRecords.some(rec => rec.startsWith(selectedDayTab));
    });
    
    const perfectList = filtered.filter(r => r.attendanceRecords && r.attendanceRecords.length >= Math.max(1, eventDaysArr.length));
    const partialList = attendedList.filter(r => !perfectList.includes(r));
    const absentList = filtered.filter(r => !r.attendanceRecords || r.attendanceRecords.length === 0);

    let tabList = filtered;
    if (detailTab === "attendance") tabList = attendedList;
    else if (detailTab === "perfect") tabList = perfectList;
    else if (detailTab === "partial") tabList = partialList;
    else if (detailTab === "absent") tabList = (selectedDayTab === "all" ? absentList : filtered.filter(r => !attendedList.includes(r)));
      ...
      <Page
        ...
        actions={
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn onClick={() => fetchRegistrations()} variant="ghost" small><Icon name="refresh" size={14} /> Refresh</Btn>
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: selectedDayTab === "all" && eventDaysArr.length > 0 ? "repeat(4, 1fr)" : "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
          {(selectedDayTab === "all" && eventDaysArr.length > 0 ? [
            { id: "registrations", label: "Total Registered", value: eventRegs.length, color: "#0B1F3B" },
            { id: "perfect", label: "Perfect Attendees (All Days)", value: perfectAttendedCount, color: "#10b981" },
            { id: "partial", label: "Partial Attendees (Some Days)", value: tabAttendedCount - perfectAttendedCount, color: "#3b82f6" },
            { id: "absent", label: "Absentees (No Days)", value: eventRegs.filter(r => !r.attendanceRecords || r.attendanceRecords.length === 0).length, color: "#f59e0b" }
          ] : [
            { id: "registrations", label: "Total Registered", value: eventRegs.length, color: "#0B1F3B" },
            { id: "attendance", label: selectedDayTab === "all" ? "Attended (Any Day)" : `Attended (${selectedDayTab})`, value: tabAttendedCount, color: "#059669" },
            { id: "absent", label: selectedDayTab === "all" ? "Absent / Pending" : `Absent (${selectedDayTab})`, value: tabAbsentCount, color: "#f59e0b" }
          ]).map(s => (
            <div 
              key={s.label} 
              onClick={() => setDetailTab(s.id)}
              style={{ 
                background: "#fff", 
                borderRadius: 14, 
                padding: "18px 22px", 
                boxShadow: detailTab === s.id ? `0 0 0 2px ${s.color}33, 0 4px 14px rgba(0,0,0,0.1)` : "0 2px 12px rgba(0,0,0,0.06)", 
                borderLeft: `4px solid ${s.color}`,
                cursor: "pointer",
                transition: "all 0.2s",
                transform: detailTab === s.id ? "translateY(-2px)" : "none"
              }}
            >
        ...
        <div style={{ background: "#fff", borderRadius: 14, padding: 16, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 20 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            {["registrations", "attendance", "perfect", "partial", "absent"].map(tab => {
              if (selectedDayTab !== "all" && ["perfect", "partial"].includes(tab)) return null;
              if (eventDaysArr.length === 0 && ["perfect", "partial"].includes(tab)) return null;
              if (selectedDayTab === "all" && eventDaysArr.length > 0 && tab === "attendance") return null;

              let label = "";
              if (tab === "registrations") label = `All (${eventRegs.length})`;
              if (tab === "attendance") label = `Attended (${tabAttendedCount})`;
              if (tab === "perfect") label = `Perfect (${perfectAttendedCount})`;
              if (tab === "partial") label = `Partial (${tabAttendedCount - perfectAttendedCount})`;
              if (tab === "absent") label = `Absent (${tabAbsentCount})`;
              ...
              selectedDayTab === "all" ? (
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {eventDaysArr.map(day => {
                    ...
                    const attended = r.attendanceRecords && r.attendanceRecords.some(rec => rec.trim().toLowerCase().startsWith(dayNorm));
                    return (
                      <span
                        key={day}
                        style={{
                          ...
                          color: attended ? "#059669" : "#9ca3af",
                          border: attended ? "1px solid #10b981" : "1px solid #e5e7eb",
                          whiteSpace: "nowrap"
                        }}
                      >
                        {attended ? "✓ " : ""}{day}
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", maxWidth: 200 }}>
                  {dbEvent.eventDays.split(",").map(dayStr => {
                    ...
                    const dayNorm = day.trim().toLowerCase();
                    const isAttended = r.attendanceRecords && r.attendanceRecords.some(rec => rec.trim().toLowerCase().startsWith(dayNorm));
                    return (
                      <button
                        key={day}
